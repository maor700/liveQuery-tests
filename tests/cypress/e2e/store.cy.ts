...
describe('liveQuery reactivity tests', () => {
  ...
  beforeEach(async () => {
    ...
    await sleep(2000);
    populateDB();
    items_A_SpyOn = cy.spy(mockCallbackObj, 'items_A');
    items_B_SpyOn = cy.spy(mockCallbackObj, 'items_B');
  });
  ...
  //   );
  //   await sleep();
  //   expect(items_A_SpyOn).to.be.callCount(1);
  //   expect(items_B_SpyOn).to.be.callCount(1);
  // });
  ...
  describe('Putting all in the same liveQuery pass the tests.', () => {
    it('Adding item will trigger only the layer associated with it by layerId', async () => {
      ...
      await sleep();
      expect(items_A_SpyOn).to.be.callCount(3); // one for first time subscription and the others becuse db mutations;
      expect(items_B_SpyOn).to.be.callCount(1); // one for first time subscription
    });
  });
  describe('Working with seperated queries and RX,working also.', () => {
    it('Adding item will trigger only the layer associated with it by layerId', async () => {
      ...
      await sleep();
      expect(items_A_SpyOn).to.be.callCount(4);
      expect(items_B_SpyOn).to.be.callCount(2);
    });
  });
});
